# This script calculates differences in regional participation coefficient
# between anxiety subgroups and plots these differences a brainplot for four
# different thresholds (2%, 5%, 10%, and 20%).
...
#-------------------------------------------------------------------------
participation_regional <- matrix(nrow = nrow(subjects), ncol = 367)
participation_regional <- as.data.frame(participation_regional)
participation_regional[,8:367] <- data2$par
names(participation_regional)[names(participation_regional) == 'V1'] <- 'cluster'
participation_regional$cluster <- 'HC'
participation_regional$cluster[] <- lapply(participation_regional$cluster, as.character)
class(participation_regional$cluster) <- "character"
names(participation_regional)[names(participation_regional) == 'V2'] <- 'age'
participation_regional$age <- HC_stats_table$age
names(participation_regional)[names(participation_regional) == 'V3'] <- 'sex'
participation_regional$sex <- HC_stats_table$sex
names(participation_regional)[names(participation_regional) == 'V4'] <- 'framewise_displacement'
participation_regional$framewise_displacement <- HC_stats_table$framewise_displacement
names(participation_regional)[names(participation_regional) == 'V5'] <- 'medication'
participation_regional$medication <- HC_stats_table$medication
names(participation_regional)[names(participation_regional) == 'V6'] <- 'medication_binary'
participation_regional$medication_binary <- HC_stats_table$medication_binary
names(participation_regional)[names(participation_regional) == 'V7'] <- 'dataset'
participation_regional$dataset <- HC_stats_table$dataset
participation_regional2 <- participation_regional
...
participation_regional <- matrix(nrow = nrow(subjects), ncol = 367)
participation_regional <- as.data.frame(participation_regional)
participation_regional[,8:367] <- data$par
names(participation_regional)[names(participation_regional) == 'V1'] <- 'cluster'
participation_regional$cluster <- data$clusterlabels
participation_regional$cluster[] <- lapply(participation_regional$cluster, as.character)
class(participation_regional$cluster) <- "character"
names(participation_regional)[names(participation_regional) == 'V2'] <- 'age'
participation_regional$age <- stats_table$age
names(participation_regional)[names(participation_regional) == 'V3'] <- 'sex'
participation_regional$sex <- stats_table$sex
names(participation_regional)[names(participation_regional) == 'V4'] <- 'framewise_displacement'
participation_regional$framewise_displacement <- stats_table$framewise_displacement
names(participation_regional)[names(participation_regional) == 'V5'] <- 'medication'
participation_regional$medication <- stats_table$medication
names(participation_regional)[names(participation_regional) == 'V6'] <- 'medication_binary'
participation_regional$medication_binary <- stats_table$medication_binary
names(participation_regional)[names(participation_regional) == 'V7'] <- 'dataset'
participation_regional$dataset <- stats_table$dataset

participation_regional <- rbind(participation_regional, participation_regional2) #add healthy controls

#-----------stats-----------
my_lms <- lapply(8:367, function(i) lm((participation_regional[,i] - mean(participation_regional[,i])) ~ participation_regional$cluster + participation_regional$age + participation_regional$sex + participation_regional$framewise_displacement + participation_regional$medication_binary + participation_regional$dataset))
...
names(participation_regional) <- c(colnames(participation_regional[,1:7]),extra_labels$extra_labels,r_extra_labels$r_extra_labels)
...
rstat_adj <- rstat
rstat_adj[,3] <- p.adjust(rstat[,3], method = 'BH')
...
ggarrange(Fig1, Fig2, Fig3, Fig4, labels = c("A", "B", "C", "D"), ncol=2, nrow=2, common.legend = TRUE, legend="right") +
  ggtitle("Regional differences in participation between anxiety subgroups for different thresholds \n") + 
