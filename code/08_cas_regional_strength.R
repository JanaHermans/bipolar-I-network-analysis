# This script calculates differences in regional strength between anxiety
# subgroups and plots these differences a brainplot for four different
# thresholds (2%, 5%, 10%, and 20%).
...
#-------------------------------------------------------------------------
strength_regional <- matrix(nrow = nrow(subjects), ncol = 367)
strength_regional <- as.data.frame(strength_regional)
strength_regional[,8:367] <- data2$strength
names(strength_regional)[names(strength_regional) == 'V1'] <- 'cluster'
strength_regional$cluster <- 'HC'
strength_regional$cluster[] <- lapply(strength_regional$cluster, as.character)
class(strength_regional$cluster) <- "character"
names(strength_regional)[names(strength_regional) == 'V2'] <- 'age'
strength_regional$age <- HC_stats_table$age
names(strength_regional)[names(strength_regional) == 'V3'] <- 'sex'
strength_regional$sex <- HC_stats_table$sex
names(strength_regional)[names(strength_regional) == 'V4'] <- 'framewise_displacement'
strength_regional$framewise_displacement <- HC_stats_table$framewise_displacement
names(strength_regional)[names(strength_regional) == 'V5'] <- 'medication'
strength_regional$medication <- HC_stats_table$medication
names(strength_regional)[names(strength_regional) == 'V6'] <- 'medication_binary'
strength_regional$medication_binary <- HC_stats_table$medication_binary
names(strength_regional)[names(strength_regional) == 'V7'] <- 'dataset'
strength_regional$dataset <- HC_stats_table$dataset
strength_regional2 <- strength_regional
...
strength_regional <- matrix(nrow = nrow(subjects), ncol = 367)
strength_regional <- as.data.frame(strength_regional)
strength_regional[,8:367] <- data$strength
names(strength_regional)[names(strength_regional) == 'V1'] <- 'cluster'
strength_regional$cluster <- data$clusterlabels
strength_regional$cluster[] <- lapply(strength_regional$cluster, as.character)
class(strength_regional$cluster) <- "character"
names(strength_regional)[names(strength_regional) == 'V2'] <- 'age'
strength_regional$age <- stats_table$age
names(strength_regional)[names(strength_regional) == 'V3'] <- 'sex'
strength_regional$sex <- stats_table$sex
names(strength_regional)[names(strength_regional) == 'V4'] <- 'framewise_displacement'
strength_regional$framewise_displacement <- stats_table$framewise_displacement
names(strength_regional)[names(strength_regional) == 'V5'] <- 'medication'
strength_regional$medication <- stats_table$medication
names(strength_regional)[names(strength_regional) == 'V6'] <- 'medication_binary'
strength_regional$medication_binary <- stats_table$medication_binary
names(strength_regional)[names(strength_regional) == 'V7'] <- 'dataset'
strength_regional$dataset <- stats_table$dataset

strength_regional <- rbind(strength_regional, strength_regional2) #add healthy controls

#-----------stats-----------
my_lms <- lapply(8:367, function(i) lm((strength_regional[,i] - mean(strength_regional[,i])) ~ strength_regional$cluster + strength_regional$age + strength_regional$sex + strength_regional$framewise_displacement + strength_regional$medication_binary + strength_regional$dataset))
...
names(strength_regional) <- c(colnames(strength_regional[,1:7]),extra_labels$extra_labels,r_extra_labels$r_extra_labels)
...
ggarrange(Fig1, Fig2, Fig3, Fig4, labels = c("A", "B", "C", "D"), ncol=2, nrow=2, common.legend = TRUE, legend="right") +
  ggtitle("Regional differences in strength between anxiety subgroups for different thresholds \n") + 
